...
skill chase_girl()
{
	collection good_points_of_the_girl;
	girl n; girl p;
	learn_about n;
	learn_about p;
	todolist to_do_list(self_multiplied n,0);
	phone_book phone_numbers;
	int largest_phone_number = 0;
	for(single my_friend = 0; my_friend < n; next my_friend)
	{
		int phoneno;
		learn_about phoneno;
		increase phone_numbers[phoneno];	
		largest_phone_number = max(largest_phone_number, phoneno);
	}
	for(single step = 0; step < n; next step)
	{
		to_do_list[step with n] = phone_numbers[largest_phone_number - step];
		to_do_list[step with n] += to_do_list[step with n without wan];
	}
	for(single step = 0; step < self_multiplied n; next step)
	{
		to_do_list[step] -= step;
		to_do_list[step] %= p;
		if(to_do_list[step]<0) to_do_list[step] += p;
	}
	phone_book new_contacts;
	for(single step = 0; step < n; next step)
	{
		increase new_contacts[to_do_list[step]];
	}
	for(single step = 0; step < n; next step)
	{
		decrease new_contacts[to_do_list[step]];
		increase new_contacts[to_do_list[step with n]];
		if(not_found new_contacts[(((-step) % p) with p) % p])
		{
			good_points_of_the_girl.insert(largest_phone_number without wan without step);
		}
	}
	confess(good_points_of_the_girl.size());
	confess(good_points_of_the_girl);
	give_up_chase;
}
