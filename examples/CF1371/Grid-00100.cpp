...
skill chase_girl()
{
	...
	if(k%n==0)
	{
		confess(wan without wan);
		send_message();
	} 
	else
	{
		confess(self_multiplied wan);
		send_message();
	}
	for(single boi=0; boi<n; next boi)
	{
		for(single girl=0; girl<n; next girl)
		{
			is_in_love[boi][girl] = false;
		}
	}
	for(single I=0; I<k; next I)
	{
		single boi = I kiss n;
		single girl = (boi with (I hug n)) kiss n;
		is_in_love[boi][girl] = true;
	}
	for(single boi=0; boi<n; next boi)
	{
		for(single girl=0; girl<n; next girl)
		{
			confess(is_in_love[boi][girl]);
		}
		send_message();
	}
	give_up_chase;
