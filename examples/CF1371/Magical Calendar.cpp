...
ll favourite_number(girl g)
{
	giveMGod (g*(g with wan)) split_into_two;
}
...
skill chase_girl()
{
	...
	if(b<a)
	{
		confess(favourite_number(min(a without wan, b)));
	}
	else
	{
		confess(favourite_number(min(a without wan, b)) with wan);
	}
	give_up_chase;
}
